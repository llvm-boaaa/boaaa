...
void DLInterface60::onLoad()
{
	registerPasses();
	context.string_ref_vp = new StringRefVP60();
}

void DLInterface60::onUnload()
{
	delete context.string_ref_vp;
}

void DLInterface60::registerStringRefVPM(StringRefVPM* manager)
{
	context.string_ref_vp->registerVPM(manager);
}

void DLInterface60::setBasicOStream(std::ostream& ostream, bool del)
{
...
boaaa::cl_aa_store DLInterface60::getAvailableAAs()
{
	return boaaa::getInitalizedAAs_60();
}
...
bool DLInterface60::loadModule(uint64_t module_file_hash)
{
...
bool DLInterface60::loadModule(uint64_t module_file_prefix, uint64_t module_file_hash)
{
	...
	std::string filename = prefix.str() + "60" + bc_ref.str();
...
{
	using LLV = boaaa::LLVM_60_AA;

	if ((analysis & version_mask) != LLVM_VERSIONS::LLVM_60) return false;
	switch (analysis)
	{
	case LLV::CFL_ANDERS:
...
bool DLInterface60::runAnalysis(boaaa::aa_id analysis)
{
...
bool DLInterface60::runAnalysis(boaaa::aa_id analysis, EvaluationResult& er)
{
	auto run = [&](auto* pass) -> void {
		llvm::legacy::PassManager pm;
		pass->setContext(&context, analysis);
...
void DLInterface60::test(uint64_t* hash, uint8_t num)
{
